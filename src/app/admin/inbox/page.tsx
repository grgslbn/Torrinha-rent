...
const CLASSIFICATION_COLORS: Record<string, string> = {
  payment_query: "bg-t-accent-light text-t-accent-text",
  ...
  other: "bg-t-bg text-t-text-muted",
};
      ...
      <div className="flex gap-4" style={{ minHeight: "70vh" }}>
        ...
        <div className={`${selected ? "w-1/2" : "w-full"} transition-all`}>
          {loading ? (
            ...
            <div className="bg-t-surface border border-t-border rounded-[var(--t-radius-lg)] p-8 text-center text-t-text-muted">
              ...
              {items.map((item) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-t-bg transition-colors ${
                    selected?.id === item.id ? "bg-t-accent-light" : ""
                  }`}
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {item.classification && (
                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium shrink-0 ${CLASSIFICATION_COLORS[item.classification] ?? "bg-t-bg text-t-text-muted"}`}>
                    ...
                    {/* Time */}
                    <span className="text-xs text-t-text-muted ml-auto shrink-0">
                      {timeAgo(item.received_at)}
                    </span>
                  </div>
                  <p className="text-sm text-t-text-secondary truncate">{item.subject || "(no subject)"}</p>
                  <p className="text-xs text-t-text-muted truncate mt-0.5">
        ...
        {/* Right: detail panel */}
        {selected && (
          <div className="w-1/2 bg-t-surface border border-t-border rounded-[var(--t-radius-lg)] flex flex-col overflow-hidden">
            {/* Urgent banner */}
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-4 border-b border-t-border">
                <h3 className="text-xs font-medium text-t-text-muted uppercase mb-2">Original Email</h3>
                <div className="text-sm mb-1">
                  <span className="font-medium text-t-text-secondary">From:</span>{" "}
                  <span className="text-t-text">{selected.from_name || "(no name)"} &lt;{selected.from_email || "(no email)"}&gt;</span>
                </div>
                <div className="text-sm mb-2">
                  <span className="font-medium text-t-text-secondary">Subject:</span>{" "}
                  <span className="text-t-text">{selected.subject || "(no subject)"}</span>
                </div>
                <pre className="text-sm text-t-text-secondary whitespace-pre-wrap font-sans bg-t-bg rounded-[var(--t-radius-sm)] p-3 max-h-48 overflow-y-auto">
              ...
              {/* Draft reply */}
              <div className="p-4 border-b border-t-border">
                <h3 className="text-xs font-medium text-t-text-muted uppercase mb-2">Draft Reply</h3>
                {editing ? (
                  <>
                    <input
                      type="text"
                      ...
                      className="w-full px-3 py-1.5 border border-t-border rounded-[var(--t-radius-sm)] text-sm text-t-text mb-2"
                    />
                    <textarea
                      value={editBody}
                      onChange={(e) => setEditBody(e.target.value)}
                      rows={8}
                      className="w-full px-3 py-2 border border-t-border rounded-[var(--t-radius-sm)] text-sm text-t-text"
                    />
                  </>
                ) : (
                  <>
                    <p className="text-sm font-medium text-t-text mb-1">{editSubject}</p>
                    <pre className="text-sm text-t-text-secondary whitespace-pre-wrap font-sans bg-t-accent-light rounded-[var(--t-radius-sm)] p-3 max-h-48 overflow-y-auto">
              ...
              {selected.claude_reasoning && (
                <div className="p-4 border-b border-t-border">
                  <h3 className="text-xs font-medium text-t-text-muted uppercase mb-1">Claude&apos;s Reasoning</h3>
                  <p className="text-xs text-t-text-muted italic">{selected.claude_reasoning}</p>
                </div>
              )}
            </div>
            ...
            {selected.status === "pending" && (
              <div className="p-4 border-t border-t-border flex gap-2 bg-t-bg">
                <button
                  onClick={handleSend}
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm text-t-text-secondary bg-t-surface border border-t-border rounded-[var(--t-radius-sm)] hover:bg-t-bg"
                >
                  {editing ? "Preview" : "Edit"}
                </button>
                <button
                  onClick={handleDismiss}
                  className="px-4 py-2 text-sm text-t-text-muted hover:text-t-text hover:bg-t-bg rounded-[var(--t-radius-sm)] ml-auto"
                >
                  Dismiss
                </button>
                <button
                  onClick={handleDelete}
                  className="px-4 py-2 text-sm text-red-500 hover:text-red-700 hover:bg-red-50 rounded-[var(--t-radius-sm)]"
                >
                  Delete
                </button>
              </div>
            ...
            {selected.status === "sent" && (
              <div className="p-4 border-t border-t-border bg-green-50 text-sm text-green-700 flex items-center justify-between">
            ...
            {selected.status === "dismissed" && (
              <div className="p-4 border-t border-t-border bg-t-bg text-sm text-t-text-muted flex items-center justify-between">
                <span>Dismissed</span>
                <button onClick={handleDelete} className="text-xs text-red-500 hover:text-red-700">Delete</button>
              </div>
            )}
